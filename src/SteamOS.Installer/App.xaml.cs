...
namespace SteamOSConfigurator
{
    public partial class App : Application
    {
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            {
                Logger.Log($"[INSTALLER FATAL CRASH] {ev.ExceptionObject}");
            };
            ...
            this.DispatcherUnhandledException += (s, ev) => 
            {
                try
                {
                    Logger.Log($"[INSTALLER WPF EXCEPTION] {ev.Exception}");
                    MessageBox.Show($"Error en el Asistente de Instalación:\n\n{ev.Exception.Message}", "Instalador de SteamOS", MessageBoxButton.OK, MessageBoxImage.Error);
                }
                catch { }
                ev.Handled = true;
            };
