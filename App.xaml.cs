    ...
    public partial class App : System.Windows.Application
    {
        ...
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            else 
            {
                if (!EsAdministrador())
                {
                    try { Process.Start(new ProcessStartInfo { UseShellExecute = true, WorkingDirectory = Environment.CurrentDirectory, FileName = Environment.ProcessPath, Verb = "runas" }); } catch { }
        ...
        private async Task EjecutarModoConsolaAsync()
        {
            try
            {
                ...
                if (config != null && config.EmuladorActivado) _ = TraductorMando.IniciarAsync();
