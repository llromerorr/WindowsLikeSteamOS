...
namespace SteamOSConfigurator
{
    ...
    public partial class App : System.Windows.Application
    {
        ...
        static extern int ChangeDisplaySettingsExReset(string? lpszDeviceName, IntPtr lpDevMode, IntPtr hwnd, int dwflags, IntPtr lParam);
        ...
        {
            Window ventanaOculta = new Window
            {
                Width = 0, Height = 0,
            ...
            try
            {
                ...
                foreach (string deviceName in activos)
                {
                    if (deviceName == config.MonitorDeviceName)
                    {
                        ...
                        ChangeDisplaySettingsEx(deviceName, ref mode, IntPtr.Zero, CDS_UPDATEREGISTRY | CDS_SET_PRIMARY | CDS_NORESET, IntPtr.Zero);
                    }
                }
                ...
                foreach (string deviceName in activos)
                {
                    if (deviceName != config.MonitorDeviceName)
                    {
                        ...
                        ChangeDisplaySettingsEx(deviceName, ref modeDetach, IntPtr.Zero, CDS_UPDATEREGISTRY | CDS_NORESET, IntPtr.Zero);
                    }
                }
            ...
            try
            {
                ...
                foreach (var kvp in _monitoresOriginales)
                {
                    ...
                    if (mode.dmPositionX == 0 && mode.dmPositionY == 0)
                    {
                        ...
                        ChangeDisplaySettingsEx(kvp.Key, ref mode, IntPtr.Zero, CDS_UPDATEREGISTRY | CDS_SET_PRIMARY | CDS_NORESET, IntPtr.Zero);
                    }
                }
                foreach (var kvp in _monitoresOriginales)
                {
